...
class DataValidation:
    ...
    def  validate_no_of_columns(self, dataframe: pd.DataFrame)-> bool:
        try:
            ...
            logging.info(f"Required number of columns:{no_of_cols}")
    ...
    def initiate_data_validation(self) -> Optional[DataValidationArtifact]:
        try:
            ...
            if not status:
                error_message = f"Train Dataframe has some missing columns."

            status = self.validate_no_of_columns(dataframe=test_dataframe)
            if not status:
                error_message = f"Test Dataframe has some missing columns."
